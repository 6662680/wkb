        ...
        <form action="" method="post" class="form form-horizontal" id="form-member-add" novalidate="novalidate">
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-3">
                    <input type="text" class="input-text" value="<?php echo ($adminInfo['uname']); ?>" name="uname" <?php if($actionName == '后台用户编辑'): ?>disabled<?php endif; ?>>
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>密码:</label>
                <div class="formControls col-xs-8 col-sm-3">
                    <input type="password" class="input-text" value="" name="pword" placeholder="<?php if($actionName == '后台用户编辑'): ?>不填,则保持原密码<?php endif; ?>">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-3">
                    <select class="select" name="role_id">
                        <?php if(is_array($roleList)): foreach($roleList as $key=>$vo): ?><option value="<?php echo ($vo["role_id"]); ?>" <?php if($adminInfo['role_id'] == $vo['role_id']): ?>selected<?php endif; ?>><?php echo ($vo["rname"]); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            </div>
            <div class="row cl">
