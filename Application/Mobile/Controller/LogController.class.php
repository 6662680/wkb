...
//
        ...
        if (!$rst = $model->where(['mobile' => $post['mobile']])->find()) {
			...
			if($rst['deviceid_mobile'] != $post['deviceid'] && !empty($rst['deviceid_mobile'])){
				$jpush = new \Vendor\Jpush\Jpush(C('JPUSHMOBILEAPPKEY'),C('JPUSHMOBILEMASTERSECRET'));
				$push = [
					'type' => 9,
					'msg' => '该账号已在另一台设备上登陆,如果不是您本人操作，请立刻修改密码',
                    'user_id' => $rst['id'],
				];
				 $jpush->push($rst['deviceid_mobile'],$push, '只是一个测试');
			}
        }
        ...
        $update = $model->where(['mobile' => $post['mobile']])->save($save);

        if ($update === FALSE) {
            returnajax(FALSE, '', '数据更新失败');
        }
...
//        pr(md5($post['user_id'] . $post['mobile'] . $post['token']));
//        echo '分割线';
//        pr($post['md5']);
        ...
        if ($post['deviceid'] != $rst['deviceid_mobile'] && !empty($rst['deviceid_mobile'])) {
            ...
            $rst = $jpush->push($rst['deviceid_mobile'], $push, '只是一个测试');
